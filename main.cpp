...
clearstring(BGZF *bgzip_file, const std::string &outstring_allp, bool is_output_to_stdout = false,
    ...
    if (TIsInputCompressed) {
        ...
        LOG(logINFO) << "Written " << ret << " bytes of compressed data from " << outstring_allp.size()
...
load_refstring(const faidx_t *ref_faidx, uvc1_refgpos_t tid, uvc1_refgpos_t incbeg, uvc1_refgpos_t excend) {
    ...
    assertUVC (regionlen == (excend - incbeg) ||
               !fprintf(stderr, "%d == %u - %u failed for %s:%d-%d\n", regionlen, excend, incbeg, tname, incbeg,
                        excend));
...
bgzip_string(std::string &compressed_outstring, const std::string &uncompressed_outstring) {
    ...
    do {
        if (compressed_totlen + BGZF_BLOCK_SIZE >= compressed_capacity) {
            char *compressed_outstr_tmp = (char *) realloc(compressed_outstr,
                                                           (compressed_capacity * 2 + BGZF_BLOCK_SIZE) * sizeof(char));
        ...
        bgzf_compress(compressed_outstr + compressed_totlen, &compressed_len,
                      uncompressed_outstring.c_str() + uncompress_totlen, block_len, 5);
...
rescue_variants_from_vcf(
    ...
    for (const auto &bedline: bedlines) {
        ...
        regionstring += (std::get<0>(tname_tseqlen_tuple) + ":" + std::to_string(rpos_inclu_beg + 1) + "-" +
                         std::to_string(rpos_exclu_end + 1 - 1));
    }
    ...
    if (sr_set_opt_retval < 0) {
        LOG(logCRITICAL) << "The validation of the index file for the tumor vcf " << vcf_tumor_fname
                         << " failed with return code " << sr_set_opt_retval;
        exit(-8);
    }
    int sr_add_reader_retval = bcf_sr_add_reader(sr, vcf_tumor_fname.c_str());
    if (sr_add_reader_retval != 1) {
        LOG(logCRITICAL) << "Failed to synchronize-read the tumor vcf " << vcf_tumor_fname << " with return code "
                         << sr_add_reader_retval;
        exit(-9);
    }
    ...
    while (bcf_sr_next_line(sr)) {
        ...
        for (uint32_t i = 1; i < line->n_allele; i++) {
            if ('<' == line->d.allele[i][0] && ((strcmp("<NON_REF>", line->d.allele[i]) &&
                                                 strcmp("<ADDITIONAL_INDEL_CANDIDATE>", line->d.allele[i])) ||
        ...
        assertUVC((2 == ndst_val && 2 == valsize) ||
                  !fprintf(stderr, "2 == %d && 2 == %d failed for VTI and line %ld!\n", ndst_val, valsize, line->pos));
        assertUVC(
                (2 == line->n_allele) || !fprintf(stderr, "Bcf line %ld has %d alleles!\n", line->pos, line->n_allele));
        const AlignmentSymbol symbol = AlignmentSymbol(bcfints[1]);

        auto symbolpos = ((isSymbolSubstitution(symbol) || MGVCF_SYMBOL == symbol ||
                           ADDITIONAL_INDEL_CANDIDATE_SYMBOL == symbol) ? (line->pos) : (line->pos + 1));
        TumorKeyInfo tki;
        tki.VTI = bcfints[1];
        ...
        if (MGVCF_SYMBOL != symbol && ADDITIONAL_INDEL_CANDIDATE_SYMBOL != symbol) {
            ...
            */
            ndst_val = 0;
            valsize = bcf_get_format_int32(bcf_hdr, line, "bDPf", &bcfints, &ndst_val);
            assertAlways((2 == ndst_val && 2 == valsize) ||
                         !fprintf(stderr, "2 == %d && 2 == %d failed for bDPf and line %ld!\n", ndst_val, valsize,
                                  line->pos));
            tki.bDP = bcfints[1];
            ndst_val = 0;
            valsize = bcf_get_format_int32(bcf_hdr, line, "bDPr", &bcfints, &ndst_val);
            assertAlways((2 == ndst_val && 2 == valsize) ||
                         !fprintf(stderr, "2 == %d && 2 == %d failed for bDPr and line %ld!\n", ndst_val, valsize,
                                  line->pos));
            tki.bDP += bcfints[1];
            ...
            assertAlways((2 == ndst_val && 2 == valsize) ||
                         !fprintf(stderr, "2 == %d && 2 == %d failed for CDP1x and line %ld!\n", ndst_val, valsize,
                                  line->pos));
            tki.CDP1x = bcfints[0];
            ...
            assertAlways((2 == ndst_val && 2 == valsize) ||
                         !fprintf(stderr, "2 == %d && 2 == %d failed for cVQ1 and line %ld!\n", ndst_val, valsize,
                                  line->pos));
            tki.cVQ1 = bcfints[1];
            ...
            assertAlways((2 == ndst_val && 2 == valsize) ||
                         !fprintf(stderr, "2 == %d && 2 == %d failed for cPCQ1 and line %ld!\n", ndst_val, valsize,
                                  line->pos));
            tki.cPCQ1 = bcfints[1];
            ...
            assertAlways((2 == ndst_val && 2 == valsize) ||
                         !fprintf(stderr, "2 == %d && 2 == %d failed for CDP2x and line %ld!\n", ndst_val, valsize,
                                  line->pos));
            tki.CDP2x = bcfints[0];
            ...
            assertAlways((2 == ndst_val && 2 == valsize) ||
                         !fprintf(stderr, "2 == %d && 2 == %d failed for cVQ2 and line %ld!\n", ndst_val, valsize,
                                  line->pos));
            tki.cVQ2 = bcfints[1];
            ...
            assertAlways((2 == ndst_val && 2 == valsize) ||
                         !fprintf(stderr, "2 == %d && 2 == %d failed for cPCQ2 and line %ld!\n", ndst_val, valsize,
                                  line->pos));
            tki.cPCQ2 = bcfints[1];
            ...
            assertAlways((2 == ndst_val && 2 == valsize) ||
                         !fprintf(stderr, "2 == %d && 2 == %d failed for bNMQ and line %ld!\n", ndst_val, valsize,
                                  line->pos));
            tki.bNMQ = bcfints[1];

            ndst_val = 0;
            valsize = bcf_get_format_int32(bcf_hdr, line, "vHGQ", &bcfints, &ndst_val);
            assertAlways((1 == ndst_val && 1 == valsize) ||
                         !fprintf(stderr, "1 == %d && 1 == %d failed for vHGQ and line %ld!\n", ndst_val, valsize,
                                  line->pos));
            tki.vHGQ = bcfints[0];
...
rescue_hotspots_from_vcf(const std::string & hotspot_vcf_fname) {
    ...
    int sr_set_opt_retval = bcf_sr_set_opt(sr, BCF_SR_REQUIRE_IDX);
    if (sr_set_opt_retval < 0) {
        LOG(logCRITICAL) << "The validation of the index file for the tumor vcf " << hotspot_vcf_fname
                         << " failed with return code " << sr_set_opt_retval;
        exit(-8);
    }
    int sr_add_reader_retval = bcf_sr_add_reader(sr, hotspot_vcf_fname.c_str());
    if (sr_add_reader_retval != 1) {
        LOG(logCRITICAL) << "Failed to synchronize-read the tumor vcf " << hotspot_vcf_fname << " with return code "
                         << sr_add_reader_retval;
        exit(-9);
    }
    ...
    while (bcf_sr_next_line(sr)) {
        ...
        const auto retkey = std::make_tuple(line->rid, symbolpos, symbol);
        ret.insert(std::make_pair(retkey, std::vector<std::string>()));
...
region_repeatvec_to_baq_offsetarr(
    ...
    for (auto i = extended_inclu_beg_pos; i < extended_exclu_end_pos; i++) {
        ...
        if (tracklen2 / rtr.unitlen >= 3 ||
            (tracklen2 / rtr.unitlen >= 2 && tracklen2 >= (uvc1_readpos_t) round(paramset.indel_polymerase_size))) {
...
bool
are_depths_diff(uvc1_readnum_t currDP, uvc1_readnum_t prevDP, uvc1_readnum_t mul_perc_ratio,
                uvc1_readnum_t add_num_ratio) {
...
process_batch(
    ...
    bool is_loginfo_enabled = (ispowerof2(regionbatch_ordinal + 1) ||
                               ispowerof2(regionbatch_tot_num - regionbatch_ordinal) || paramset.always_log);
    ...
    if (is_loginfo_enabled) {
        LOG(logINFO) << "Thread " << thread_id
                     << " starts bamfname_to_strand_to_familyuid_to_reads with pair_end_merge = "
                     << paramset.pair_end_merge;
    }
    ...
    if (is_loginfo_enabled) {
        LOG(logINFO) << "Thread " << thread_id << " starts converting umi_to_strand_to_reads with is_by_capture = "
                     << is_by_capture << "  ";
    }
    ...
    const uvc1_qual_t minABQ_snv = ((ASSAY_TYPE_AMPLICON == inferred_assay_type) ? paramset.syserr_minABQ_pcr_snv
                                                                                 : paramset.syserr_minABQ_cap_snv);
    const uvc1_qual_t minABQ_indel = ((ASSAY_TYPE_AMPLICON == inferred_assay_type) ? paramset.syserr_minABQ_pcr_indel
                                                                                   : paramset.syserr_minABQ_cap_indel);
    ...
    const uvc1_refgpos_t extended_inclu_beg_pos = MAX(0, non_neg_minus(MIN(incluBegPosition, bam_inclu_beg_pos),
                                                                       MAX_STR_N_BASES));
    const uvc1_refgpos_t extended_exclu_end_pos = MIN(std::get<1>(tname_tseqlen_tuple),
                                                      MAX(excluEndPosition, bam_exclu_end_pos) + MAX_STR_N_BASES);

    const auto tkis_beg = tid_pos_symb_to_tkis.lower_bound(
            std::make_tuple(tid, extended_inclu_beg_pos, AlignmentSymbol(0)));
    const auto tkis_end = tid_pos_symb_to_tkis.upper_bound(
            std::make_tuple(tid, extended_exclu_end_pos + 1, AlignmentSymbol(0)));
    ...
    if (is_loginfo_enabled) {
        LOG(logINFO) << "Thread " << thread_id << " deals with " << num_rescued << " tumor-sample variants in region "
                     << extended_inclu_beg_pos << " to " << extended_exclu_end_pos + 1;
    }

    if (is_loginfo_enabled) {
        LOG(logINFO) << "Thread " << thread_id << " starts constructing symbolToCountCoverageSet12 with "
    ...
    if (is_loginfo_enabled) {
        LOG(logINFO) << "Thread " << thread_id << " starts updateByRegion3Aln with " << umi_strand_readset.size()
                     << " families";
    }
    ...
    std::vector<RegionalTandemRepeat> region_repeatvec = refstring2repeatvec(
            ...
            0);
    const auto &baq_offsetarr = region_repeatvec_to_baq_offsetarr(region_repeatvec, tid, extended_inclu_beg_pos,
                                                                  extended_exclu_end_pos + 1, paramset);
    const auto &baq_offsetarr2 = region_repeatvec_to_baq_offsetarr<true>(region_repeatvec, tid, extended_inclu_beg_pos,
                                                                         extended_exclu_end_pos + 1, paramset);

    assertUVC(((baq_offsetarr.getExcluEndPosition() - baq_offsetarr.getIncluBegPosition()) ==
               UNSIGN2SIGN(region_repeatvec.size()))
              ||
              !fprintf(stderr, "%d - %d == %lu failed (baq == repeat in size)!\n", baq_offsetarr.getExcluEndPosition(),
                       baq_offsetarr.getIncluBegPosition(), region_repeatvec.size()));
    ...
    if (paramset.inferred_is_vcf_generated) {
        ...
        for (uvc1_refgpos_t zerobased_pos = rpos_inclu_beg; zerobased_pos <= rpos_exclu_end; zerobased_pos++, prev_tracklen = curr_tracklen) {
            ...
            const std::array<AlignmentSymbol, 2> symboltype_to_refsymbol = {{
                ...
                ? BASE_NN : CHAR_TO_SYMBOL.data.at(refstring.at((zerobased_pos - 1) - extended_inclu_beg_pos))), LINK_M, }};
            const auto refsize = UNSIGN2SIGN(refstring.size());
            const auto refidx = zerobased_pos - extended_inclu_beg_pos;
            const AlignmentSymbol prev_base1 = ((refidx >= 2) ? CHAR_TO_SYMBOL.data.at(refstring.at((refidx - 2)))
                                                              : BASE_NN);
            const AlignmentSymbol prev_base2 = ((refidx >= 3) ? CHAR_TO_SYMBOL.data.at(refstring.at((refidx - 3)))
                                                              : BASE_NN);
            ...
            for (const SymbolType symboltype: SYMBOL_TYPE_ARR) {
                ...
                if ((paramset.outvar_flag & OUTVAR_MGVCF) && ((((refpos) % MGVCF_REGION_MAX_SIZE) == 0) || (refpos == incluBegPosition))
                    ...
                    const auto rp2end = MIN(refpos + MGVCF_REGION_MAX_SIZE + 1,
                                            symbolToCountCoverageSet12.getUnifiedExcluEndPosition());
                    for (uvc1_refgpos_t rp2 = refpos; rp2 < rp2end; rp2++) {
                        for (SymbolType stype: SYMBOL_TYPES_IN_VCF_ORDER) {
                            ...
                            for (int strand = 0; strand < 2; strand++) {
                                curr_tot_bdepth += formatSumBySymbolType(frag_format_depth_sets[strand].getByPos(rp2),
                                                                         stype, FRAG_bDP);
                                curr_tot_cdepth += formatSumBySymbolType(fam_format_depth_sets[strand].getByPos(rp2),
                                                                         stype, FAM_cDP1);
                                curr_tot_cdep12 += formatSumBySymbolType(fam_format_depth_sets[strand].getByPos(rp2),
                                                                         stype, FAM_cDP12);
                            }
                            ...
                            const auto ref_like_binom = -calc_binom_10log10_likeratio(paramset.contam_any_mul_frac,
                                                                                      nonref_cdepth + 0.5,
                                                                                      curr_tot_cdepth + 1.0);
                            ...
                            const auto nonref_like_binom = -calc_binom_10log10_likeratio(paramset.germ_hetero_FA,
                                                                                         ref_cdepth + 0.5,
                                                                                         curr_tot_cdepth + 1.0);
                    ...
                    const std::string gvcf_blockline = string_join(std::vector<std::string>{{
                                                                                                    ...
                                                                                                    std::string(".") +
                                                                                                    ":" + std::to_string(match_refsymbol) +
                                                                                                    "," + std::to_string(MGVCF_SYMBOL) +
                                                                                                    ":" + int32t_join(pos_stype_BDP_CDP_refQ_1dvec,",") + "," +
                                                                                                    std::to_string(rp2end),
                                                                                            }}, "\t");
                    ...
                    if (paramset.is_tumor_format_retrieved && IS_PROVIDED(paramset.vcf_tumor_fname)) {
                        ...
                        if (tkis_it != tid_pos_symb_to_tkis.end()) {
                            const auto &tkis = tid_pos_symb_to_tkis.find(
                                    std::make_tuple(tid, refpos, MGVCF_SYMBOL))->second;
                            if (tkis.size() == 1) {
                                tumor_gvcf_format = bcf1_to_string(bcf_hdr, tkis[0].bcf1_record);
                                LOG(logDEBUG4) << "gVCFblock at " << refpos << " is indeed found, tumor_gvcf_format == "
                                               << tumor_gvcf_format;
                            } else {
                                tumor_gvcf_format = std::string("\t.:.,.:-1");
                                LOG(logDEBUG4) << "gVCFblock at " << refpos << " is not found, tkis.size() == "
                                               << tkis.size();
                            }
                        } else {
                ...
                const auto aCDP = symbolToCountCoverageSet12.seg_format_prep_sets.getByPos(
                ...
                if ((OUTVAR_ADDITIONAL_INDEL_CANDIDATE & paramset.outvar_flag)
                    ...
                    const std::string vcfline = string_join(std::vector<std::string>{{
                                                                                             ...
                                                                                             std::string("."), // qual
                                                                                             std::string("."), // filter
                                                                                             (std::string("ADDITIONAL_INDEL_CANDIDATE;RU=") +
                                                                                              repeatunit + ";RC=" +
                                                                                              std::to_string(repeatnum)), // info
                                                                                             std::string("GT:VTI:clipDP"), // format
                                                                                             std::string(".") + ":" +
                                                                                             std::to_string(match_refsymbol) +
                                                                                             "," + std::to_string(ADDITIONAL_INDEL_CANDIDATE_SYMBOL)
                                                                                             + ":" + std::to_string(ADP) + "," +
                                                                                             std::to_string(aCDP) // format values
                                                                                     }}, "\t");
                    std::string tumor_format = "";
                    if (paramset.is_tumor_format_retrieved && IS_PROVIDED(paramset.vcf_tumor_fname)) {
                        const auto tkis_it = tid_pos_symb_to_tkis.find(
                                std::make_tuple(tid, refpos, ADDITIONAL_INDEL_CANDIDATE_SYMBOL));
                        if (tkis_it != tid_pos_symb_to_tkis.end()) {
                            const auto &tkis = tid_pos_symb_to_tkis.find(
                                    std::make_tuple(tid, refpos, ADDITIONAL_INDEL_CANDIDATE_SYMBOL))->second;
                            if (tkis.size() == 1) {
                ...
                const auto ref_bdepth =
                        symbolToCountCoverageSet12.symbol_to_frag_format_depth_sets[0].getByPos(
                                refpos)[refsymbol][FRAG_bDP]
                        + symbolToCountCoverageSet12.symbol_to_frag_format_depth_sets[1].getByPos(
                                refpos)[refsymbol][FRAG_bDP];

                for (AlignmentSymbol symbol: SYMBOL_TYPE_TO_SYMBOLS[symboltype]) {
                    const bool is_pos_rescued = (IS_PROVIDED(paramset.vcf_tumor_fname) &&
                                                 (extended_posidx_to_is_rescued[refpos - extended_inclu_beg_pos]));
                    const bool is_var_rescued = (is_pos_rescued && (tid_pos_symb_to_tkis.end() !=
                                                                    tid_pos_symb_to_tkis.find(
                                                                            std::make_tuple(tid, refpos, symbol))));
                    const auto bdepth =
                            symbolToCountCoverageSet12.symbol_to_frag_format_depth_sets[0].getByPos(
                                    refpos)[symbol][FRAG_bDP]
                            + symbolToCountCoverageSet12.symbol_to_frag_format_depth_sets[1].getByPos(
                                    refpos)[symbol][FRAG_bDP];
                    const auto cdepth =
                            MAX(symbolToCountCoverageSet12.symbol_to_fam_format_depth_sets_2strand[0].getByPos(
                                    refpos)[symbol][FAM_cDP1],
                                symbolToCountCoverageSet12.symbol_to_fam_format_depth_sets_2strand[0].getByPos(
                                        refpos)[symbol][FAM_cDP12])
                            + MAX(symbolToCountCoverageSet12.symbol_to_fam_format_depth_sets_2strand[1].getByPos(
                                    refpos)[symbol][FAM_cDP1],
                                  symbolToCountCoverageSet12.symbol_to_fam_format_depth_sets_2strand[1].getByPos(
                                          refpos)[symbol][FAM_cDP12]);
                    ...
                    const auto indices_fq = (simplemut2indices_fq.find(simplemut) != simplemut2indices_fq.end()
                                             ? simplemut2indices_fq[simplemut] : empty_size_t_set);
                    const auto indices_f2q = (simplemut2indices_f2q.find(simplemut) != simplemut2indices_f2q.end()
                                              ? simplemut2indices_f2q[simplemut] : empty_size_t_set);
                    ...
                    if (isSymbolIns(symbol) || isSymbolDel(symbol)) {
                        for (int strand = 0; strand < 2; strand++) {
                            if (0 < symbolToCountCoverageSet12.symbol_to_frag_format_depth_sets[strand].getByPos(
                                    refpos)[symbol][FRAG_bDP]) {
                                fill_by_indel_info(
                                        fmt,
                                        symbolToCountCoverageSet12,
                        ...
                        bcad0a_indelstring_tki_vec.push_back(
                                std::make_tuple(bdepth, cdepth, std::string(""), THE_DUMMY_TUMOR_KEY_INFO));
                    }
            ...
            for (SymbolType symboltype: SYMBOL_TYPE_ARR) {
                ...
                for (auto &fmt_tki_tup: fmt_tki_tup_vec) {
                    ...
                    assertUVC((VTI >= 0 && VTI <= END_ALIGNMENT_SYMBOLS) ||
                              !fprintf(stderr, "VTI %d at pos %d is invalid!\n", VTI, refpos));
                    ...
                    if (refsymbol != symbol) {
                        ...
                        maxVQ_VQ1_VQ2_symbol_indelstr_tup_vec.push_back(
                                std::make_tuple(MAX(cVQ1, cVQ2), cVQ1, cVQ2, symbol,
                                                LAST(std::get<0>(fmt_tki_tup).gapSa)));
                        const uvc1_qual_t germ_phred_het3al = ((BASE_SYMBOL == symboltype)
                                                               ? paramset.germ_phred_het3al_snp
                                                               : paramset.germ_phred_het3al_indel);
                ...
                for (auto &fmt_tki_tup: fmt_tki_tup_vec) {
                    ...
                    for (const auto &maxVQ_VQ1_VQ2_symbol_indelstr_tup: maxVQ_VQ1_VQ2_symbol_indelstr_tup_vec) {
                        ...
                        std::get<0>(fmt_tki_tup).cVQAM[tup_vec_idx] = SYMBOL_TO_DESC_ARR[std::get<3>(
                                maxVQ_VQ1_VQ2_symbol_indelstr_tup)];
                ...
                for (auto &fmt_tki_tup: fmt_tki_tup_vec) {
                    ...
                    if (paramset.should_add_note) {
                        fmt.note += std::string("/symb/") + std::to_string(symbol) +
                                    std::string("/gVQ1/CONTQ/") + std::to_string(LAST(fmt.gVQ1)) + "/" +
                                    std::to_string(LAST(fmt.CONTQ)) + "//";
                    }
                    if (symbol != BASE_NN) {
            ...
            for (const auto symboltype: SYMBOL_TYPE_ARR) {
                ...
                for (auto &fmt_tki_tup: fmt_tki_tup_vec) {
                    ...
                    const bool will_generate_out = (ISNT_PROVIDED(paramset.vcf_tumor_fname) ?
                            (paramset.outvar_flag & OUTVAR_ANY) : (tki.ref_alt.size() > 0 && (paramset.outvar_flag & OUTVAR_SOMATIC)));
                    const bool is_out_blocked = (((BASE_NN == symbol) && !(OUTVAR_BASE_NN & paramset.outvar_flag))
                    ...
                    if (will_generate_out && !is_out_blocked) {
                        ...
                        const auto nlodq_singlesample = nlodq_singlesite - 3 + (isSymbolSubstitution(symbol) ?
                                paramset.germ_phred_hetero_snp : paramset.germ_phred_hetero_indel);
                        ...
                        if (IS_PROVIDED(paramset.vcf_tumor_fname)) {
                            ...
                            const auto fmtptrs = std::vector<bcfrec::BcfFormat *>{
                                    {std::get<1>(nlodq_fmtptr1_fmtptr2_tup), std::get<2>(nlodq_fmtptr1_fmtptr2_tup)}};
                            for (const auto *fmtptr: fmtptrs) {
                                ...
                                uvc1_qual_t binom_b10log10like = calc_binom_10log10_likeratio((tDP - tAD) / (tDP),
                                                                                              nDP - nAD, nAD);
                                uvc1_qual_t powlaw_b10log10like = (paramset.powlaw_exponent * 10 / log(10) *
                                                                   log(bjpfrac));
                                uvc1_qual_t phred_het3al_chance_inc_snp =
                                        2 * paramset.germ_phred_hetero_snp - paramset.germ_phred_het3al_snp;
                                uvc1_qual_t phred_het3al_chance_inc_indel =
                                        2 * paramset.germ_phred_hetero_indel - paramset.germ_phred_het3al_indel;
                                uvc1_qual_t triallele_inc = ((normsymbol != symbol) ?
                                                             (isSymbolSubstitution(symbol) ? phred_het3al_chance_inc_snp
                                                                                           : phred_het3al_chance_inc_indel)
                                                                                    : 0);
                                uvc1_qual_t triallele_thr = 3; // 8;
                                const auto new_nlodq_inc =
                                        BETWEEN(MIN(binom_b10log10like, powlaw_b10log10like), -triallele_thr,
                                ...
                                if (nlodq_inc > new_nlodq_inc) {
                                    ...
                                    if (paramset.should_add_note) {
                                        fmt.note += std::string("/nlodqDeltaIs/")
                                                    + other_join(std::array<double, 5>{
                                                {(double) tAD, (double) tDP, (double) nAD, (double) nDP,
                                                 (double) nlodq_inc}}, "/") + "#"
...
main(int argc, char **argv) {
    ...
    const std::string UMI_STRUCT_STRING = ((UMI_STRUCT != NULL && strlen(UMI_STRUCT) > 0) ? std::string(UMI_STRUCT)
                                                                                          : std::string(""));
    ...
    BGZF *fp_pass = ((!is_vcf_out_pass_empty_string && !is_vcf_out_pass_to_stdout) ? bgzip_open_wrap1(
            paramset.vcf_out_pass_fname) : NULL);
    ...
    */
    ...
    for (size_t i = 0; i < nidxs; i++) {
        ...
        if (NULL == sam_idxs[i]) {
            LOG(logCRITICAL) << "Failed to load BAM index " << paramset.bam_input_fname << " for thread with ID = "
                             << i;
            exit(-4);
        }
        if (paramset.fasta_ref_fname.size() > 0) {
            ...
            if (NULL == ref_faidxs[i]) {
                LOG(logCRITICAL) << "Failed to load reference index for file " << paramset.fasta_ref_fname
                                 << " for thread with ID = " << i;
                exit(-5);
            }
        }
    }
    ...
    // rescue_variants_from_vcf
    tid_pos_symb_to_tkis1 = rescue_variants_from_vcf(bedlines1, tid_to_tname_tseqlen_tuple_vec,
                                                     paramset.vcf_tumor_fname, g_bcf_hdr,
    ...
    while (iter_nreads > 0) {
        ...
        std::thread read_bam_thread(
                [&bedlines2, &tid_pos_symb_to_tkis2, &samIter, &iter_nreads, &iter_ret_flag, &n_sam_iters, &paramset, &tid_to_tname_tseqlen_tuple_vec, g_bcf_hdr]() {
                    bedlines2.clear();
                    iter_nreads = samIter.iternext(iter_ret_flag, bedlines2, 0);
                    LOG(logINFO) << "PreProcessed " << iter_nreads << " reads in tier-1-region no " << (n_sam_iters);

                    tid_pos_symb_to_tkis2 = rescue_variants_from_vcf(bedlines2, tid_to_tname_tseqlen_tuple_vec,
                                                                     paramset.vcf_tumor_fname, g_bcf_hdr,
                                                                     paramset.is_tumor_format_retrieved);
                    LOG(logINFO) << "Rescued/retrieved " << tid_pos_symb_to_tkis2.size()
                                 << " variants in tier-1-region no " << (n_sam_iters);
                });
        ...
        LOG(logINFO) << "Start-bam-iteration-" << (n_sam_iters - 1)
                     << ": will-process-the-following-tier1-regions (all indices are zero-based): ";
        for (size_t t2_idx = 0; t2_idx < beg_end_pair_vec.size(); t2_idx++) {
            ...
            LOG(logINFO) << "End-of-tier-2-region-no-" << t2_idx << " tot_n_reads=" << t2_tot_n_reads
                         << " tot_n_ref_bases=" << t2_tot_n_bases;
            t1_tot_n_reads += t2_tot_n_reads;
            t1_tot_n_bases += t2_tot_n_bases;
        }
        assertUVC(((size_t) t1_tot_n_bases) == npositions ||
                  !fprintf(stderr, "%lu ==%lu failed!", ((size_t) t1_tot_n_bases), npositions));
        LOG(logINFO) << "End-of-tier-1-region-no-" << (n_sam_iters - 1) << " tot_n_reads=" << t1_tot_n_reads
                     << " tot_n_ref_bases=" << t1_tot_n_bases;
        if (bed_out.is_open()) {
            bed_out << bedstring;
        }
        ...
        LOG(logINFO) << "Start processing the chunks from " << allridx << " to " << allridx + incvalue
                     << " which contains approximately " << nreads << " reads and " << npositions
                     << " positions divided into "
